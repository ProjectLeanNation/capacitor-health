...
public class HealthPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func queryWorkouts(_ call: CAPPluginCall) {
        ...
        let includeHeartRate = call.getBool("includeHeartRate") ?? false
        let includeRoute = call.getBool("includeRoute") ?? false
        let includeSteps = call.getBool("includeSteps") ?? false
        ...
        let workoutQuery = HKSampleQuery(sampleType: HKObjectType.workoutType(), predicate: predicate, limit: HKObjectQueryNoLimit, sortDescriptors: nil) { query, samples, error in
            ...
            // Process each workout into the normalized cross-platform schema
            for workout in workouts {
                var workoutDict: [String: Any] = [
                    "startDate": self.isoDateFormatter.string(from: workout.startDate),
                    "endDate": self.isoDateFormatter.string(from: workout.endDate),
        ...
        let heartRateQuery = HKSampleQuery(sampleType: heartRateType, predicate: predicate, limit: HKObjectQueryNoLimit, sortDescriptors: nil) { query, samples, error in
            ...
            for sample in heartRateSamplesData {
                let sampleDict: [String: Any] = [
                    "timestamp": self.isoDateFormatter.string(from: sample.startDate),
        ...
        let locationQuery = HKWorkoutRouteQuery(route: route) { query, locations, done, error in
            ...
            for location in locations {
                let locationDict: [String: Any] = [
                    "timestamp": self.isoDateFormatter.string(from: location.timestamp),
    ...
    /// Maps HealthKit workout activity types to the shared SCREAMING_SNAKE schema used on Android.
    let workoutTypeMapping: [UInt: String] = [
        ...
        28: "MARTIAL_ARTS",
        29: "GUIDED_BREATHING",
        30: "HIGH_INTENSITY_INTERVAL_TRAINING",
        31: "PADDLING",
        32: "OTHER",
        33: "STRETCHING",
        34: "RACQUETBALL",
